...
while j <= 5:
    i = 0
    while i < j:  # 打印三角形，个数与行数相同，i 表示每行中*的个数，这个数字与行号相等，所以i与j进行联动
        print('*', end='')  # 打印取消默认换行，默认是end='\n'，打印成一横
        i += 1
    print()  # 结果要是一行一行的，所以得调回来，不然结果就是一行长的
    j += 1
...
while j <= 9:
    i = 1
    while i <= j:  # 公式个数与行数相同，i 表示每行中*的个数，这个数字与行号相等，所以i与j进行联动
        # print(f'{i} * x = i * x', end='') 把x改成j就行了，如下
        # print(f'{i} * {j} = {i * j}', end='')  # 但是结构不是特别好看，所以得优化一下
